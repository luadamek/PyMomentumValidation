...
output_location = os.path.join(os.getenv("MomentumValidationDir"), "SolutionHistograms_Nov24")
...
for region in ["MS", "ID"]:
    ...
    directory = "/project/def-psavard/ladamek/sagitta_bias_matrices/Injection_Nov24_{}_Data_Inject_None_v03_v2/OutputFiles".format(region,bias)
    subtraction_dir = "/project/def-psavard/ladamek/sagitta_bias_matrices/Injection_Nov24_{}_MC_Inject_{}_v03_v2/OutputFiles".format(region,"None")
    ...
    def difference_histogram(meas, injected, name, low_hi = None):
        import ROOT
        import numpy as np
        values = []
    ...
    for bias in sagitta_histograms:
         ...
         histogram, mean, std = difference_histogram(sagitta_hist, solution, bias + "Difference", low_hi = None)
         ...
         pt = ROOT.TPaveText(.15,.5,.5,.95)
         pt.AddText("Mean: {}".format(mean))
         pt.AddText("Std. Dev: {}".format(std))
         print(mean, std)
         #t1 = ROOT.TText(.2,.8,"Mean: {}".format(mean))
         #t2 = ROOT.TText(.2,.65,"Std. Dev: {}".format(std))
         pt.Draw()
         canvas.Print("{}.pdf".format(histogram.GetName()))

         name = "{}_{}".format("RemainingBias", bias)
         effect_correction = get_effect_histogram(solution, sagitta_hist, name = name)
         name = "{}_{}".format("InjectionBias", bias)
         effect_injection = get_effect_histogram(solution, name = name)

         draw_2d_histogram(effect_correction, "    #sqrt{s}= 13 TeV, Simulation", normalize = False, output_location=output_location)
         draw_2d_histogram(effect_injection, "    #sqrt{s}= 13 TeV, Simulation", normalize = False, output_location=output_location)

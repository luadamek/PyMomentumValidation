...
if __name__ == "__main__":
        ...
        def get_mean_histogram_value(histogram):
             nbins = 0
             mean = 0
             for i in range(1, histogram.GetNbinsX() + 1):
                 for j in range(1, histogram.GetNbinsY() + 1):
                     mean += histogram.GetBinContent(i, j)
                     nbins += 1
                ...
                for region in ["ID", "ME", "CB"]:
                    ...
                    for iteration in [1,2]:
                        ...
                        for method in methods:
                            ...
                            x = []
                            y = []
                            for i in range(0, matrix_round + 1):
                                 directory = base_directory.format(inject = "None", detector_location=region, round=i, method=method, file_type=data_filetype, syst_var=variation, end_string = end_string)
                                 sagitta_hist = get_sagitta_bias_histogram(directory, is_data=True)
                                 x.append(i)
                                 y.append(get_mean_histogram_value(sagitta_hist))
                            y = [y[0]] + [abs(now - before) for (before, now) in zip(y[:-1], y[1:])]
                            f = plt.figure()
                            plt.plot(x, y)
                            plt.yscale('log')
                            plt.xlabel("Iteration")
                            plt.ylabel(r"$Mean\ |\delta| [1/TeV]$")
                            f.savefig(os.path.join(output_location, "MeanValues.png"))
